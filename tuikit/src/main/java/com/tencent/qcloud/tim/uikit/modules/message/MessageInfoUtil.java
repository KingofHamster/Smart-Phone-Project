...
public class MessageInfoUtil {
    ...
    public static MessageInfo buildCustomMessage(String data) {
        ...
        info.setFromUser(V2TIMManager.getInstance().getLoginUser());
        info.setExtra(data.contains("redPackage") ? "[红包]" : "[礼物]");
        return info;
    }
        ...
        if (type == V2TIMMessage.V2TIM_ELEM_TYPE_CUSTOM) {
            ...
            if (data.equals(MessageCustom.BUSINESS_ID_GROUP_CREATE)) {
                ...
                try {
                    ...
                    GiftRedPackage giftRedPackage = liveGson.fromJson(data, GiftRedPackage.class);
                    if (giftRedPackage.getMsgType() != null) {
                        if ("gift".equals(giftRedPackage.getMsgType())) {
                            msgInfo.setExtra("[" + giftRedPackage.getGift().getName() + "]");
                        } else {
                            msgInfo.setExtra("[红包]");
                        }
                    }
